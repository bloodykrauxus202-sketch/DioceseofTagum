...
for (const file of chunkFiles) {
  ...
  // Also remove exact empty string '' assignment which causes 1769 blank parishes!
  const emptySingleQ = `parish: ''`;
  const emptyDoubleQ = `parish: ""`;
  const replaceUnassigned = `parish: 'Unassigned'`;
  if (content.includes(emptySingleQ)) {
    content = content.split(emptySingleQ).join(replaceUnassigned);
    fileChanges++;
  }
  if (content.includes(emptyDoubleQ)) {
    content = content.split(emptyDoubleQ).join(replaceUnassigned);
    fileChanges++;
  }
