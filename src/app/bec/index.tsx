...
/** Returns true if the first meaningful character is a letter A-Z */
...
/**
 * Proper A-Z comparator: letter-starting entries always sort before
 * symbol/number-starting entries. Within each group, use locale sort.
 */
 ...
 */
function keyComparator(key: keyof GKK) {
...
function getDirectionsUrl(gkk: GKK): string {
  ...
  // Plus Code or other non-empty coordinate text
...
/**
 * Generate a human-readable direction hint from the location name.
 * Mirrors the style used in parish screens.
 */
function generateDirectionHint(gkk: GKK): string {
  const addr = (gkk.address ?? '').trim();
  if (!addr || !isValidLocation(addr)) return '';

  // If it already looks like a proper paragraph / sentence, return as-is
  if (addr.length > 60) return addr;

  // Build a natural direction sentence from the address parts
  // Common format: "Purok X, Barangay, Municipality, Province"
...
function buildGkkMapHtml(gkkList: GKK[]): string {
  // Extract lat/lon from coordinates field (decimal only — no fabrication)
  const data = gkkList.flatMap(g => {
    ...
    // Sanity-check: Philippines bounding box
...
export default function GKKDirectory() {
  ...
  const filteredGKKs = useMemo(() => {
    ...
    if (sortBy === 'alphabetical') sorted.sort(gkkNameComparator);
    else if (sortBy === 'parish') sorted.sort(keyComparator('parish'));
    else sorted.sort(keyComparator('vicariate'));

    return sorted;
  }, [searchQuery, sortBy]);

  // ── Sections for alphabetical / grouped views ───────────────────────────────
  const sections = useMemo(() => {
    const groups: { key: string; data: GKK[] }[] = [];

    if (sortBy === 'alphabetical') {
      // Separate letter-starting vs non-letter-starting
      const letterGroups: Record<string, GKK[]> = {};
      const otherGroup: GKK[] = [];

      filteredGKKs.forEach(gkk => {
        if (startsWithLetter(gkk.name)) {
          const letter = gkk.name.trimStart().charAt(0).toUpperCase();
          if (!letterGroups[letter]) letterGroups[letter] = [];
          letterGroups[letter].push(gkk);
        } else {
          otherGroup.push(gkk);
        }
      });
      ...
      Object.keys(letterGroups)
        ...
        .forEach(letter => groups.push({ key: letter, data: letterGroups[letter] }));

      // Then symbols / numbers
      if (otherGroup.length > 0) {
        groups.push({ key: '#', data: otherGroup });
      }
    } else {
      const keyField = sortBy === 'parish' ? 'parish' : 'vicariate';
      const map: Record<string, GKK[]> = {};
      filteredGKKs.forEach(gkk => {
        const k = String(gkk[keyField] || '(Unassigned)');
        if (!map[k]) map[k] = [];
        map[k].push(gkk);
      });
      ...
      Object.keys(map)
        ...
        .forEach(k => groups.push({ key: k, data: map[k] }));
    }
    return groups;
  }, [filteredGKKs, sortBy]);

  // ── Flat list items (header + cards) ───────────────────────────────────────
  type ListItem =
    | { type: 'header'; key: string; label: string }
    | { type: 'card'; gkk: GKK; flatIndex: number };
    ...
    sections.forEach(section => {
      const label =
        sortBy === 'parish'
          ? `Parish: ${section.key}`
          : sortBy === 'vicariate'
          ? `Vicariate: ${section.key}`
          : section.key;
      items.push({ type: 'header', key: section.key, label });
      section.data.forEach(gkk => {
        items.push({ type: 'card', gkk, flatIndex: globalIndex++ });
      });
    });
    return items;
  }, [sections, sortBy]);

  // ── Map HTML (only built when map is active) ────────────────────────────────
  const mapHtml = useMemo(
    () => (view === 'map' ? buildGkkMapHtml(filteredGKKs) : ''),
    [view, filteredGKKs]
  );

  const mappedCount = useMemo(
    () =>
      filteredGKKs.filter(g => {
        const coords = (g.coordinates ?? '').replace(/[()]/g, '').trim();
    ...
    <View style={{ flex: 1, backgroundColor: '#f9fafb' }}>
      ...
      <View style={{ backgroundColor: '#fff', borderBottomWidth: 1, borderBottomColor: '#e5e7eb', paddingTop: insets.top }}>
        ...
        <View style={{ flexDirection: 'row', alignItems: 'center', paddingHorizontal: 12, paddingVertical: 8 }}>
          ...
          <View style={{ flex: 1, marginLeft: 8 }}>
            <View style={{ flexDirection: 'row', alignItems: 'center' }}>
              <Text style={{ fontSize: 21, fontWeight: '700', color: '#111827' }}>BECs / GKKs</Text>
              <View style={{
            ...
            <Text style={{ fontSize: 14, color: '#9ca3af' }}>
              {filteredGKKs.length === gkkData.length
                ? `${gkkData.length.toLocaleString()} GKKs total`
                : `${filteredGKKs.length.toLocaleString()} of ${gkkData.length.toLocaleString()} GKKs`}
            </Text>
          </View>

          {/* Map / List toggle */}
          <View style={{ flexDirection: 'row', backgroundColor: '#e0e7ff', borderRadius: 12, padding: 3, gap: 2 }}>
            <Pressable
            ...
            >
              ...
              <Text style={{ fontSize: 15, fontWeight: '700', color: view === 'list' ? '#1d4ed8' : '#6b7280' }}>List</Text>
            </Pressable>
            <Pressable
              onPress={() => setView('map')}
              style={{
            ...
            >
              ...
              <Text style={{ fontSize: 15, fontWeight: '700', color: view === 'map' ? '#1d4ed8' : '#6b7280' }}>Map</Text>
            </Pressable>
          </View>
        </View>
        ...
        <View style={{ paddingHorizontal: 12, paddingBottom: 8 }}>
          <View style={{ flexDirection: 'row', alignItems: 'center', backgroundColor: '#f3f4f6', borderRadius: 12, paddingHorizontal: 12, paddingVertical: 8, gap: 8 }}>
            <Search size={16} color="#9ca3af" />
            <TextInput
              style={{ flex: 1, fontSize: 17, color: '#111827' }}
              placeholder="Search GKK, patron, parish..."
        ...
        {/* Sort tabs — only shown in List view */}
        {view === 'list' && (
          <ScrollView
            horizontal
          ...
          >
            {(['alphabetical', 'parish', 'vicariate'] as const).map(s => (
              <Pressable
                key={s}
                onPress={() => setSortBy(s)}
                style={{
                  paddingHorizontal: 12, paddingVertical: 5, borderRadius: 20,
                  backgroundColor: sortBy === s ? '#1d4ed8' : '#f3f4f6',
                }}
              >
                <Text style={{ fontSize: 14, fontWeight: '600', color: sortBy === s ? '#fff' : '#4b5563' }}>
                  {s === 'alphabetical' ? 'A – Z' : s === 'parish' ? 'By Parish' : 'By Vicariate'}
                </Text>
              </Pressable>
            ))}
          </ScrollView>
          ...
          <View style={{ backgroundColor: '#eff6ff', borderBottomWidth: 1, borderBottomColor: '#bfdbfe', paddingHorizontal: 16, paddingVertical: 9 }}>
            <Text style={{ fontSize: 15, color: '#1d4ed8', textAlign: 'center', fontWeight: '600' }}>
              {mappedCount > 0
                ...
                : 'No GKKs with coordinates in this filter'}
            </Text>
          </View>
      ...
      {view === 'list' && (
        <FlatList
          ...
          keyExtractor={(item, idx) =>
            item.type === 'header' ? `h-${item.key}-${idx}` : `c-${item.gkk.id}`
          }
          ...
          renderItem={({ item }) => {
            if (item.type === 'header') {
              return (
                <View style={{ paddingHorizontal: 4, paddingTop: 10, paddingBottom: 6 }}>
                  <Text style={{ fontSize: 13, fontWeight: '700', color: '#9ca3af', textTransform: 'uppercase', letterSpacing: 1 }}>
                    {item.label}
                  </Text>
                </View>
              );
            ...
            return (
              <GKKCard
                gkk={item.gkk}
                onPress={() => setSelectedGKK(item.gkk)}
          ...
          ListEmptyComponent={
            <View style={{ alignItems: 'center', paddingTop: 64 }}>
              ...
              <Text style={{ color: '#9ca3af', fontSize: 20, marginTop: 12 }}>No GKKs found</Text>
            </View>
          }
        />
      )}
